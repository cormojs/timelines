...
const bufEq = (a: Uint8Array | null | undefined, b: Uint8Array | null | undefined): boolean =>
...
const isConflictCopyPath = (rel: unknown): boolean => /-conflict-\d{8}-[\w.-]*\.timeline$/i.test(String(rel || ''));

const stripExt = (rel: string): string => rel.replace(/\.timeline$/i, '');
...
const README_MARKER = '<!-- generated by Timelines -->';
const LEGACY_README_HEADING = '# Timelines library';

// Only a README the app wrote may be rewritten or removed; the heading covers pre-marker repos
function isGeneratedReadme(content: unknown): boolean {
  const text = String(content || '');
  return text.includes(README_MARKER) || text.trimStart().startsWith(LEGACY_README_HEADING);
}

function parseGitHubRemote(input: unknown): { owner: string; repo: string; htmlUrl: string; cloneUrl: string } | null {
...
function viewerDeepLink(owner: string, repo: string, ref: string, relPath: string): string {
...
function summarizePackageDiff(oldBuf: Uint8Array, newBuf: Uint8Array): string | null {
  let oldPkg;
  let newPkg;
  let oldData;
  let newData;
  ...
  const countChanged = <T>(a: Record<string, T>, b: Record<string, T>, eq: (left: T, right: T) => boolean): number => {
    let n = 0;
...
type GitError = { code?: string; message?: string; cause?: { code?: string }; data?: { statusCode?: number } };

const isNetworkError = (err: GitError): boolean => {
...
const isAuthError = (err: GitError): boolean =>
  err?.code === 'HttpError' && (err?.data?.statusCode === 401 || err?.data?.statusCode === 403);

const isEmptyRemoteError = (err: GitError): boolean =>
...
class GitSyncEngine {
  ...
  _setStatus(statusState: string, err: Error | null = null): void {
  ...
  setAutoSync(enabled: boolean, debounceMs?: number): void {
  ...
  _isExcluded(relId: string): boolean {
  ...
  async connect({ url, branch = 'main' }: { url: string; branch?: string }) {
  ...
  _applyIdentity(credentials: GitSyncCredentials | null): void {
  ...
  async mirrorSize(): Promise<number> {
  ...
  async _repoVisibility(github: { owner: string; repo: string }): Promise<boolean | null> {
  ...
  async fileHistory(uid: string, { depth = 100 }: { depth?: number } = {}) {
    if (!this.isConnected()) throw new Error('Git sync is not connected');
    const rel = this.state?.uidToPath?.[uid];
    if (!rel) throw new Error('Timeline is not synced to the remote');
    const github = parseGitHubRemote(this.state.url);
  ...
  async restoreVersion(uid: string, commitOid: string) {
    if (!this.isConnected()) throw new Error('Git sync is not connected');
    const rel = this.state?.uidToPath?.[uid];
    if (!rel) throw new Error('Timeline is not synced to the remote');
  ...
  async _resolve(ref: string): Promise<string | null> {
    try {
  ...
  async _connect({ url, branch }: { url: string; branch: string }) {
    ...
    try {
      ...
      if (await this._shouldRegenerateReadme(pathsBefore !== pathsAfter)) {
        await this._regenerateReadme();
      }
      await this._stageAndCommit(this._commitMessage(summaries));
      await this._pushWithRetry(branch, new Set<string>());
  ...
  async _syncPass({ full = false }: { full?: boolean } = {}) {
  ...
  async _exportPass(dirtyUids: Set<string>, sweepAll: boolean) {
    const summaries: Array<{ path: string; summary: string }> = [];
    const exportedPaths = new Set<string>();
  ...
  _commitMessage(summaries: Array<{ path: string; summary: string }>): string | null {
  ...
  async _stageAndCommit(message: string | null) {
    let matrix = null;
    try {
  ...
  async _fetchAndReconcile(branch: string, exportedPaths: Set<string>) {
    try {
      await git.fetch({ ...this._net, remote: 'origin', ref: branch, singleBranch: true, tags: false });
    } catch (err) {
    ...
    const remoteByPath = new Map(remoteChanges.map((c: GitChange) => [c.path, c]));
  ...
  async _pushWithRetry(branch: string, exportedPaths: Set<string>): Promise<void> {
    const localOid = await this._resolve(`refs/heads/${branch}`);
    if (!localOid) return;
    const remoteOid = await this._resolve(`refs/remotes/origin/${branch}`);
  ...
  async _importPass(exportedPaths = new Set<string>(), locallyDeletedPaths = new Set<string>()) {
    const branch = this.state.branch;
    ...
    const incoming = changes.filter(
      (c: GitChange) => c.to && c.path.endsWith('.timeline') && !this._isExcluded(c.path) && !exportedPaths.has(c.path),
    );
    // Originals before conflict copies so copy imports see their uid taken
    incoming.sort(
      (a: GitChange, b: GitChange) => Number(isConflictCopyPath(a.path)) - Number(isConflictCopyPath(b.path)),
    );
  ...
  async _diffCommits(fromOid: string | null | undefined, toOid: string) {
  ...
  async _writeConflictCopy(rel: string, buf: Uint8Array): Promise<string> {
    const d = this.now();
  ...
  _mirrorAbs(rel: string): string {
    return path.join(this.repoDir, ...rel.split('/'));
  }

  async _readMirrorFile(rel: string): Promise<Buffer | null> {
    if (!rel) return null;
    try {
  ...
  async _writeMirrorFile(rel: string, buf: Uint8Array): Promise<void> {
    const abs = this._mirrorAbs(rel);
    await fsp.mkdir(path.dirname(abs), { recursive: true });
    await fsp.writeFile(abs, Buffer.from(buf));
  }

  async _deleteMirrorFile(rel: string): Promise<void> {
    await fsp.rm(this._mirrorAbs(rel), { force: true });
  }

  async _listRepoFiles(subdir = '', timelineOnly = false): Promise<string[]> {
  ...
  async _ensureRepoMetaFiles() {
    ...
    const ignoreLine = (p: string): string => (p.endsWith('/') ? `/${p}` : `/${p}.timeline`);
    const content =
      excluded.length > 0
    ...
    if (this.state?.writeReadme === false) {
      const readme = await this._readmeState();
      if (readme.generated) await fsp.rm(readme.path, { force: true });
    }
  }

  async _readmeState() {
    const readmePath = path.join(this.repoDir, 'README.md');
    if (!fs.existsSync(readmePath)) return { path: readmePath, exists: false, generated: false };
    const content = await fsp.readFile(readmePath, 'utf8').catch(() => '');
    return { path: readmePath, exists: true, generated: isGeneratedReadme(content) };
  }

  // Regenerate only when the app owns the README; a hand-written one is left alone
  async _shouldRegenerateReadme(pathsChanged: boolean): Promise<boolean> {
    if (this.state?.writeReadme === false) return false;
    const readme = await this._readmeState();
    if (readme.exists && !readme.generated) return false;
    return pathsChanged || !readme.exists;
  }
...
function createEngine(opts: GitSyncEngineOptions): GitSyncEngine {
