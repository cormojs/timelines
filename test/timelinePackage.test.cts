...
test('ESM viewer twin packageReader.ts reads stored entries', async () => {
  const { json, files } = fixture();
  const zip = buildPackage(json, files, { deterministic: true });
  const mod = await import(pathToFileURL(path.join(__dirname, '../src/utils/packageReader.ts')).href);
