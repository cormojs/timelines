...
function SectionHeader({
  ...
  return (
    <button type="button" className="edit-section-header" onClick={onToggle}>
      ...
      {!isOpen && summary && <span className="edit-section-header-summary">{summary}</span>}
    </button>
  );
}
...
export default function RightPanel({
  ...
  const stripEditableEraSuffix = useCallback(
    ...
    [timelineData?.file?.negID, timelineData?.file?.posID],
  );
  ...
  const spanParentCandidates = useMemo(() => {
    ...
    return timelineData.elements
      ...
      .filter(
        (span) =>
          Number.isFinite(span._start) &&
          Number.isFinite(span._end) &&
          parentRange.start >= span._start &&
          parentRange.start <= span._end,
      );
    ...
    return timelineData.elements
      ...
      .filter(
        (span) =>
          Number.isFinite(span._start) &&
          Number.isFinite(span._end) &&
          parentRange.end >= span._start &&
          parentRange.end <= span._end,
      );
  }, [timelineData, formData, parentRange]);
  ...
  return (
    <div ref={panelRef} className={`right-panel ${isMaximized ? 'is-maximized' : ''}`}>
      <div className="right-panel-header">
      ...
      <div className="right-panel-content">
        {!isEditMode ? (
          ...
          <div className="view-mode">
            ...
            <div className="view-group view-group-title">
              ...
              <p className="view-title-with-icon">
                {formData.icon &&
                  (() => {
                    const Icon = ICON_MAP[formData.icon];
                    return Icon ? <Icon size={14} className="view-title-icon" /> : null;
                  })()}
                {formData.title}
              </p>
            </div>
            ...
            {formData.type === 'event' ? (
              <div className="view-group">
                <label>Date</label>
                <div className="view-separator" />
                <p>{displayDateLabel(formData.dateLabel) ?? formatDisplayYear(formData.date)}</p>
              </div>
            ) : (
              <div className="view-group">
                <label>Date</label>
            ...
            {formData.type === 'event' && formData.parents && formData.parents.length > 0 && formData.parents[0] && (
              <div className="view-group">
                <label>Parent</label>
                <div className="view-separator" />
                <button type="button" className="parent-link" onClick={() => onSelect(formData.parents[0])}>
            ...
            {formData.type === 'span' && formData.parent && (
              <div className="view-group">
                <label>Parent</label>
                <div className="view-separator" />
                <button type="button" className="parent-link" onClick={() => onSelect(formData.parent)}>
            ...
            {formData.type === 'span' && formData.mergeParent && (
              <div className="view-group">
                ...
                <div className="view-separator" />
                <button type="button" className="parent-link" onClick={() => onSelect(formData.mergeParent)}>
            ...
            {formData.type === 'span' && formData.extendFrom && (
              <div className="view-group">
                ...
                <div className="view-separator" />
                <button type="button" className="parent-link" onClick={() => onSelect(formData.extendFrom)}>
            ...
            {formData.noteFile && (
              <>
                ...
                <button
                  type="button"
                  className="rp-note-header sources-collapse-btn"
                  onClick={() => setIsNoteCollapsed((v) => !v)}
                >
                  ...
                  <span className="sources-collapse-right">
                    ...
                    <ChevronDown
                      size={14}
                      style={{
                        transform: isNoteCollapsed ? 'rotate(-90deg)' : 'none',
          ...
          /* Edit Mode */
          <form id="right-panel-edit-form" className="edit-form" onSubmit={(e) => e.preventDefault()}>
            {/* Validation Errors */}
                ...
                {formData.type === 'event' ? (
                  <div className="form-group">
                    <div className="edit-row">
                      ...
                      <div className="edit-separator" />
                      <div
                        className={`edit-input-shell${showCalendarInputIcon ? ' has-left-icon has-dynamic-icon' : ''}`}
                      >
                            ...
                            {formData.parents?.[0] ? (
                              <div className="relation-selected-list">
                                <div className="relation-selected-item">
                                  ...
                                  >
                                    {timelineData.elements.find((el) => el.id === formData.parents[0])?.title ||
                                      formData.parents[0]}
                                  </button>
                                  <button
                                    type="button"
                                    ...
                                    if (e.key === 'Enter') {
                                      ...
                                      if (choice) {
                                        const next = {
                                          ...formData,
                                          parents: [choice.id],
                                          groupId: choice.groupId ?? formData.groupId,
                                        };
                                        setFormData(next);
                                        commitDraft(next);
                                        setParentQuery('');
                                        ...
                                        <button
                                          ...
                                          onMouseDown={(e) => {
                                            ...
                                            const next = {
                                              ...formData,
                                              parents: [span.id],
                                              groupId: span.groupId ?? formData.groupId,
                                            };
                                            setFormData(next);
                                            commitDraft(next);
                                            setParentQuery('');
                            ...
                            {formData.parent || formData.extendFrom ? (
                              <div className="relation-selected-list">
                                <div className="relation-selected-item">
                                  ...
                                  >
                                    {(() => {
                                      const id = formData.parent || formData.extendFrom;
                                      return timelineData.elements.find((el) => el.id === id)?.title || id;
                                    })()}
                                  </button>
                                  <button
                                    type="button"
                              ...
                              <div className="branch-picker">
                                <input
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                      e.preventDefault();
                                      const suggestions =
                                        spanRelationType === 'branch' ? spanParentSuggestions : extendFromSuggestions;
                                ...
                                />
                                {isSpanParentMenuOpen &&
                                  spanParentQuery.trim().length > 0 &&
                                  (() => {
                                    const suggestions =
                                      spanRelationType === 'branch' ? spanParentSuggestions : extendFromSuggestions;
                                    const emptyMsg =
                                      spanRelationType === 'branch' ? 'No matching spans' : 'No contiguous spans';
                                    return (
                                      <div className="branch-suggestions">
                                        {suggestions.length > 0 ? (
                                          suggestions.map((span) => (
                                            <button
                                              key={span.id}
                                              type="button"
                                              className="branch-suggestion-item"
                              ...
                              {formData.mergeParent ? (
                                <div className="relation-selected-list">
                                  <div className="relation-selected-item">
                                    ...
                                    >
                                      {timelineData.elements.find((el) => el.id === formData.mergeParent)?.title ||
                                        formData.mergeParent}
                                    </button>
                                    <button
                                      type="button"
                    ...
                    <div className="edit-row">
                      ...
                      <div className="coord-inputs">
                        <input
                          ...
                          onBlur={(e) =>
                            commitDraft({ ...formData, lat: e.target.value === '' ? null : Number(e.target.value) })
                          }
                          placeholder="lat"
                          step="any"
                        />
                        ...
                        <input
                          ...
                          onBlur={(e) =>
                            commitDraft({ ...formData, lng: e.target.value === '' ? null : Number(e.target.value) })
                          }
                          placeholder="lng"
                          step="any"
                        />
              ...
              summary={
                [
                  ...
                  (formData.color ||
                    formData.spanSize ||
                    formData.eraSize ||
                    formData.eventLineStyle ||
                    formData.eventBorderStyle ||
                    formData.hideYears ||
                    formData.hideDetails ||
                    formData.fuzzyStart ||
                    formData.fuzzyEnd) &&
                    'Styles',
                ]
                      ...
                      <div className="edit-row">
                        ...
                        <label className="settings-toggle" style={{ gridColumn: 2, justifySelf: 'end' }}>
                          <input
                            ...
                            type="checkbox"
                            checked={
                              formData.hideDetails === true ||
                              (formData.hideName === true && formData.hideYears === true)
                            }
                            onChange={(e) => {
                        ...
                        <label className="settings-toggle" style={{ gridColumn: 2, justifySelf: 'end' }}>
                          <input
                            ...
                            onChange={(e) => {
                              const next = { ...formData };
                              if (e.target.checked) {
                                next.fuzzyStart = true;
                              } else {
                                delete next.fuzzyStart;
                              }
                              setFormData(next);
                              commitDraft(next);
                            }}
                ...
                {formData.type === 'event' && (
                  <>
                    <div className="thumbnail-full-header">
                      ...
                      {formData.thumbnail && (
                        <span className="thumbnail-full-meta">
                          {(() => {
                            try {
                    ...
                    {formData.thumbnail && (
                      <div className="thumbnail-full-card">
                        <img
                        ...
                        <div className="thumbnail-full-bottombar">
                          <div className="thumbnail-style-btn">
                            <span className="thumbnail-style-label">
                              {formData.thumbnailStyle === 'banner'
                                ? 'Top banner'
                                : formData.thumbnailStyle === 'square-fill'
                                  ? 'Square fill'
                                  : formData.thumbnailStyle === 'circle-fill'
                                    ? 'Circle fill'
                                    : 'Left strip'}
                            </span>
                            <ChevronDown size={9} />
                          ...
                          <div className="thumbnail-fit-seg" role="group" aria-label="Thumbnail fit">
                            {[
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const next = { ...formData, thumbnailFit: value };
                                  setFormData(next);
                                  commitDraft(next);
                                }}
                              >
                      ...
                      <div className="thumbnail-dropzone-wrap">
                        <button
                          ...
                          onDrop={async (e) => {
                            ...
                            const filePath =
                              window.electron?.getPathForFile?.(file) ?? (file as File & { path?: string }).path;
                  ...
                  <div className="breaks-list">
                                ...
                                <div className="break-color-wrap">
                                  ...
                                  <input
                                    ...
                                    onBlur={(e) => {
                                      ...
                                      commitDraft({
                                        ...formData,
                                        breaks: nextBreaks.map((b, i) => (i === idx ? { ...b, color: normalized } : b)),
                                      });
                                    }}
                                    className="edit-color-text"
                                    maxLength={7}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const allYears = [
                          formData.start,
                          ...existingBreaks.map((b) => b.year).filter((y) => y != null),
                          formData.end,
                        ];
                        ...
                        const nextBreaks = [
                          ...existingBreaks,
                          { year: newYear, label: '', color: formData.color || '#808080' },
                        ];
                        handleChange('breaks', nextBreaks);
                        commitDraft({ ...formData, breaks: nextBreaks });
                      }}
                  ...
                  <div className="note-add-actions">
                    <div className="note-add-dropdown-wrap">
                      <button type="button" className="note-create-card" onClick={() => setIsNoteAddOpen((v) => !v)}>
                        <div className="note-create-card-icon">
                          <FileText size={18} />
                        </div>
                      ...
                      {isNoteAddOpen && (
                        <>
                          ...
                          <div className="note-add-dropdown">
                            <button
                              type="button"
                              className="note-add-dropdown-item"
                              onClick={() => {
      ...
      <div className="rp-action-bar">
        ...
        {!readOnly && (
          <div className="rp-action-group">
            <button className="rp-action-edit" type="button" onClick={toggleEditMode}>
