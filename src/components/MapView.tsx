...
function formatElementDate(el, fileConfig) {
  ...
  if (el.type === 'event') {
    const year =
      displayDateLabel(el.dateLabel) ??
      (el.date != null ? formatYear(el.date, negID, posID, useCalendar === true, hideDecimals) : null);
    return year ?? '';
  }
  const start =
    displayDateLabel(el.startLabel) ??
    (el.start != null ? formatYear(el.start, negID, posID, useCalendar === true, hideDecimals) : null);
  const end =
    displayDateLabel(el.endLabel) ??
    (el.end != null ? formatYear(el.end, negID, posID, useCalendar === true, hideDecimals) : null);
  ...
  return (
    <>
      <Rectangle
        ...
        pathOptions={REPEAT_OVERLAY_OPTIONS}
      />
      <Rectangle
        bounds={[
          [-200, 180],
    ...
    return (
      <div className="timeline-map-view">
        ...
        >
          ...
          <TileLayer url={tileUrl} attribution={tileAttribution} referrerPolicy="strict-origin-when-cross-origin" />
          <RepeatOverlay />
          {markers.map((el) => {
                  ...
                  >
                    <div className="map-popup-box" style={{ borderColor: el.resolvedColor }}>
                      <div className="map-popup-header">
                        <span className="map-popup-type" style={{ background: el.resolvedColor }}>
                      ...
                      {tags && (
                        <div className="map-popup-tags">
                          {tags.map((t) => (
                            <span key={t} className="map-popup-tag">
                              {t}
                            </span>
                          ))}
                        </div>
