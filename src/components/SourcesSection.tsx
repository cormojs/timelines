    ...
    return (
      <>
        ...
        <button type="button" className="rp-note-header sources-collapse-btn" onClick={() => setIsCollapsed((v) => !v)}>
          <span className="rp-sources-label">
            ...
            {srcList.map((src, i) => (
              <div key={`${src.title}-${i}`} className="wiki-url-card">
                ...
                <div className="wiki-url-card-info">
                  ...
                  {(src.description || src.citation) && (
                    <div className="wiki-url-card-host">{src.description || src.citation}</div>
                  )}
                </div>
                {src.url && (
                  <a
    ...
    <div className="sources-edit-section">
          ...
          {srcList.map((src, i) =>
            editingIndex === i ? (
              <div
                ...
                style={{ marginTop: 0, paddingTop: 8, borderTop: 'none' }}
              >
              ...
              <div key={`${src.title}-${i}`} className="source-item">
                ...
                <div className="source-text">
                  ...
                  {(src.description || src.citation) && (
                    <span className="source-citation">{src.description || src.citation}</span>
                  )}
                </div>
                <div className="source-item-actions">
                  ...
                  <button type="button" className="wiki-url-card-btn" onClick={() => handleEdit(i)} title="Edit">
                    <Pencil size={13} />
                  </button>
                  <button
                    type="button"
      ...
      {!isCollapsed &&
        (isFormOpen ? (
          ...
          <button type="button" className="source-add-btn" onClick={() => setIsFormOpen(true)}>
            + Add source
          </button>
        ))}
    </div>
