...
function sanitizeWikiHtml(html, host = 'https://en.wikipedia.org') {
  ...
  doc.body.querySelectorAll(mapLikeSelectors.join(',')).forEach((node) => {
    ...
    if (
      removableWrapper &&
      removableWrapper !== doc.body &&
      removableWrapper.textContent?.trim() === node.textContent?.trim()
    ) {
  ...
  doc.body
    .querySelectorAll('a[href*="geohack"], a[href*="openstreetmap"], a[href*="maplink"], a[href*="maps.wikimedia"]')
    .forEach((node) => {
  ...
  doc.body.querySelectorAll('.infobox tr').forEach((row) => {
    ...
    const looksLikeLocationList =
      row.querySelector('.plainlist, ul, ol') && /map|location|locations|coordinates|coord\./.test(rowText);
    ...
    try {
      ...
      if (!data) {
        ...
        if (pageResult?.success) {
          ...
          const apiOriginOk = (() => {
            try {
              return new URL(apiBase).origin === parsed.host;
            } catch {
              return false;
            }
          })();
          if (apiBase && apiOriginOk) {
            const discoveryTitles = [...new Set([...(discoveredTitle ? [discoveredTitle] : []), ...titleCandidates])];
    ...
    return (
      <>
        ...
        <button
          ...
          onClick={() => setIsWikiCollapsed((v) => !v)}
        >
          ...
          <span className="sources-collapse-right">
            ...
            <span
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') handleWikiRefresh(e);
              }}
            >
              <RotateCw size={13} />
            </span>
            <ChevronDown
              size={14}
              style={{
                transform: isWikiCollapsed ? 'rotate(-90deg)' : 'none',
    ...
    <>
      ...
      {isWikiUrlInputOpen && (
        <div className="wiki-url-input-card">
          ...
          <div className="source-add-actions">
            ...
            <button type="button" className="btn-primary" onClick={handleWikiUrlSubmit}>
              Save
            </button>
          </div>
        </div>
      ...
      {wikiUrl &&
        ...
        (() => {
          ...
          let articleHost = '';
          try {
            articleHost = new URL(parsedWiki.host).hostname;
          } catch {
            /* invalid host */
          }
          return (
            <div className="wiki-url-card">
              ...
              <div className="wiki-url-card-info">
                <div className="wiki-url-card-title">
                  {articleTitle}
              ...
              <div className="wiki-url-card-actions">
                <button type="button" className="wiki-url-card-btn" onClick={handleOpenWikiInput} title="Change">
                  <Pencil size={13} />
                </button>
                <button
                  type="button"
