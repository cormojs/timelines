      ...
      <span className="sb-row-title">
        {item.icon &&
          (() => {
            ...
            return Icon ? <Icon size={11} className="sb-row-icon" /> : null;
          })()}
        {item.title}
      </span>
...
function ElementRow({
  ...
  const parentSpanColor = !isSpan && !isEra && element.parents?.[0] ? spanById?.get(element.parents[0])?.color : null;
  const glyphColor =
    isEra || isSpan ? element.color || tagColor || 'var(--ui-muted)' : parentSpanColor || tagColor || 'var(--ui-muted)';
  const dateText =
    isSpan || isEra
  ...
  return (
    ...
    >
      <span className="sb-el-glyph" style={{ color: isSelected ? 'var(--text-primary)' : glyphColor }}>
        {isSpan ? (
          <span
            className="sb-el-glyph-span-bar"
            style={{ background: isSelected ? 'var(--text-primary)' : glyphColor }}
          />
        ) : isEra ? (
          <span
            className="sb-el-glyph-era-box"
            style={{ borderColor: isSelected ? 'var(--text-primary)' : glyphColor }}
          />
        ) : (
          '●'
        )}
...
export default function Sidebar({
  ...
  const commitDisplayGroupOrder = (nextDisplayGroups: TimelineGroup[], dividerIndex?: number) => {
    ...
    const nextGroups = groups.map((group) =>
      patchedById.has(group.id) ? { ...group, ...patchedById.get(group.id) } : group,
    );
  ...
  const eraRoots = useMemo(() => [...eras].sort((a, b) => a.start - b.start), [eras]);
  const spanRows = useMemo(
    () => [...spans].sort((a, b) => (a.start === b.start ? a.title.localeCompare(b.title) : a.start - b.start)),
    [spans],
  );
  const eventRows = useMemo(() => [...events].sort((a, b) => a.date - b.date), [events]);
  ...
  const visibleEras = applySidebarSort(searchActive ? eraRoots.filter((e) => elMatches(e)) : eraRoots, 'start');
  const visibleSpans = applySidebarSort(searchActive ? spanRows.filter((s) => elMatches(s)) : spanRows, 'start');
  const visibleEvents = applySidebarSort(searchActive ? eventRows.filter((ev) => elMatches(ev)) : eventRows, 'date');

  const visibleGroups = searchActive
    ? eraGroups.groups
        .map((group) => {
          const eraMatches = elMatches(group.era);
          const filteredItems = eraMatches ? group.items : group.items.filter((el) => elMatches(el));
          const filteredSubGroups = (group.subGroups || [])
            .map((subGroup) => {
              const subEraMatches = elMatches(subGroup.era);
              return {
                ...subGroup,
                items: subEraMatches ? subGroup.items : subGroup.items.filter((el) => elMatches(el)),
              };
            })
  ...
  const visibleSpanGroups = searchActive
    ? spanGroups.groups
        ...
        .filter(Boolean)
    : spanGroups.groups;
  const visibleSpanUngrouped = searchActive ? spanGroups.ungrouped.filter((el) => elMatches(el)) : spanGroups.ungrouped;
  ...
  const visibleTags = searchActive ? allTags.filter((tag) => matchesSearch(tag)) : allTags;
        ...
        <div key={era.id} className="sb-era-group">
          <div className="sb-era-header">
            ...
            <button
              ...
              onContextMenu={(e) => {
                e.preventDefault();
                e.stopPropagation();
                setElementMenu({ x: e.clientX, y: e.clientY, element: era });
              }}
            >
              <span
          ...
          {eraGroups.tree.length > 1 && (
            <div className="sb-era-bar-track">
              <div
                className="sb-era-bar"
                style={{
                  left: `${barLeft}%`,
                  width: `${barWidth}%`,
                  background: getEraLabelColor(era.color, sidebarBgHex) || eraColor,
                }}
              />
            </div>
          )}
    ...
    return (
      <div key={era.id} className="sb-sub-era-group">
        <div className="sb-sub-era-header">
          ...
          >
            <span
              className={`sb-sub-era-name${selectedId === era.id ? ' is-selected' : ''}`}
              style={
                selectedId === era.id ? undefined : { color: getEraLabelColor(era.color, sidebarBgHex) || undefined }
              }
            >
  ...
  return (
    <div className="sidebar-root">
      ...
      {timelineMenu && (
        ...
        >
          <button className="context-menu-item" onClick={() => handleMenuAction(() => onBackToHome?.())}>
            <ArrowLeft size={16} />
          ...
          <button className="context-menu-item" onClick={() => handleMenuAction(() => onNewTimeline?.())}>
            <FilePlus size={16} />
          ...
          <button className="context-menu-item" onClick={() => handleMenuAction(() => onDuplicateTimeline?.())}>
            <Copy size={16} />
            <span>Duplicate</span>
          </button>
          ...
          <button className="context-menu-item" onClick={() => handleMenuAction(() => onDownloadPng?.())}>
            <Image size={16} />
            <span>Download .png</span>
          </button>
          <button className="context-menu-item" onClick={() => handleMenuAction(() => onDownloadVideo?.())}>
            <Video size={16} />
              ...
              <div className="sb-new-wrapper" ref={newMenuRef}>
                ...
                {newMenuOpen && (
                  <div className="sb-new-menu">
                    <button
                      className="sb-new-menu-item"
                      title={formatKeybind(keybinds.newEra) ? `New Era (${formatKeybind(keybinds.newEra)})` : 'New Era'}
                      onClick={() => {
                        setNewMenuOpen(false);
                        onAddEra?.();
                      }}
                    >
                      <span className="sb-new-menu-icon">
                        <span
                          style={{
                      ...
                      Era
                      {formatKeybind(keybinds.newEra) && (
                        <span className="sb-new-menu-shortcut">{formatKeybind(keybinds.newEra)}</span>
                      )}
                    </button>
                    <button
                      className="sb-new-menu-item"
                      title={
                        formatKeybind(keybinds.newSpan) ? `New Span (${formatKeybind(keybinds.newSpan)})` : 'New Span'
                      }
                      onClick={() => {
                        setNewMenuOpen(false);
                        onAddSpan?.();
                    ...
                    >
                      ...
                      Span
                      {formatKeybind(keybinds.newSpan) && (
                        <span className="sb-new-menu-shortcut">{formatKeybind(keybinds.newSpan)}</span>
                      )}
                    </button>
                    <button
                      className="sb-new-menu-item"
                      title={
                        formatKeybind(keybinds.newEvent)
                          ? `New Event (${formatKeybind(keybinds.newEvent)})`
                          : 'New Event'
                      }
                      onClick={() => {
                        setNewMenuOpen(false);
                    ...
                    >
                      ...
                      Event
                      {formatKeybind(keybinds.newEvent) && (
                        <span className="sb-new-menu-shortcut">{formatKeybind(keybinds.newEvent)}</span>
                      )}
                    </button>
                  </div>
                )}
                ...
                {sortMenuOpen && (
                  <div className="sb-sort-menu timeline-context-menu">
                    ...
                    <button
                      className="context-menu-item"
                      onClick={() => {
                        onPatchFile?.({ panelSortField: 'year' });
                    ...
                    <button
                      className="context-menu-item"
                      onClick={() => {
                        onPatchFile?.({ panelSortField: 'name' });
                        setSortMenuOpen(false);
                      }}
                    >
                      <span>Name</span>
                    ...
                    <button
                      className="context-menu-item"
                      onClick={() => {
                        onPatchFile?.({ panelSortOrder: 'asc' });
                        setSortMenuOpen(false);
                      }}
                    >
                      <span>Ascending</span>
                    ...
                    <button
                      className="context-menu-item"
                      onClick={() => {
                        onPatchFile?.({ panelSortOrder: 'desc' });
                        setSortMenuOpen(false);
                      }}
                    >
                    ...
                    {[
                      ...
                      <button
                        ...
                        onClick={() => {
                          onPatchFile?.({
                            panelGroupMode: value,
                            nestEraSubGroups: value !== 'eras' ? false : file?.nestEraSubGroups,
                          });
                          setSortMenuOpen(false);
                        }}
                      >
                    ...
                    {file?.panelGroupMode === 'eras' && (
                      <>
                        ...
                        <button
                          className="context-menu-item"
                          onClick={() => {
                            onPatchFile?.({ panelGroupMode: 'eras', nestEraSubGroups: !file?.nestEraSubGroups });
                          }}
                        >
          ...
          <div className={`sidebar-content${sidebarTab !== 'timeline' ? ' is-tags-tab' : ''}`} ref={listRef}>
            {sidebarTab === 'timeline' ? (
              file?.panelGroupMode === 'spans' ? (
                    ...
                    const renderSpanHeader = (
                      ...
                      <div className={`sb-sub-era-header${isRoot ? ' is-root-span' : ''}`}>
                        <button className="sb-era-toggle" onClick={onToggle}>
                          <ChevronDown
                            className={`sb-caret ${isOpenState ? 'open' : ''}`}
                            size={isRoot ? 11 : 10}
                            strokeWidth={2.5}
                          />
                        </button>
                        <button
                          ...
                          onContextMenu={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            setElementMenu({ x: e.clientX, y: e.clientY, element: s });
                          }}
                        >
                          <span
                        ...
                        <span className="sb-sub-era-range">
                          {formatRange(s.start, s.end, s.startLabel, s.endLabel)}
                        </span>
                      </div>
                    );
                    return (
                      <div key={span.id} className="sb-sub-era-group">
                        {renderSpanHeader(span, true, isOpen, () =>
                          setOpenSpanGroups((prev) => ({ ...prev, [span.id]: !isOpen })),
                        )}
                        {isOpen && (
                          <div className="sb-sub-era-items">
                          ...
                          return (
                            <>
                              <div className="sb-sub-era-header is-root-span">
                                <button
                                  className="sb-era-toggle"
                                  onClick={() => setOpenSpanGroups((prev) => ({ ...prev, __other__: !isOtherOpen }))}
                                >
                                  <ChevronDown
                                    className={`sb-caret ${isOtherOpen ? 'open' : ''}`}
                                    size={11}
                                    strokeWidth={2.5}
                                  />
                                </button>
                      ...
                      <div className="sb-section-body">
                        {visibleEvents.map((ev) => (
                          <SidebarRow
                            key={ev.id}
                            item={ev}
                            rightText={displayDateLabel(ev.dateLabel) ?? fmtYear(ev.date)}
                            level={0}
                            selectedId={selectedId}
                            onSelect={onSelect}
                            listRef={listRef}
                ...
                <div className="sb-era-groups">
                  {file?.nestEraSubGroups ? sortedVisibleEraTree.map((node) => renderEraTreeNode(node, 0)) : null}
                  {!file?.nestEraSubGroups &&
                    sortedVisibleGroups.map(({ era, items, subGroups, barLeft, barWidth }) => {
                      const isOpen = searchActive || openEraGroups[era.id] !== false;
                      const eraColor = era.color || 'var(--ui-muted)';
                                  ...
                                  <div key={subEra.id} className="sb-sub-era-group">
                                    <div className="sb-sub-era-header">
                                      <button
                                        className="sb-era-toggle"
                                        onClick={() =>
                                          setOpenEraGroups((prev) => ({ ...prev, [subEra.id]: !isSubOpen }))
                                        }
                                      >
                                        <ChevronDown
                                          className={`sb-caret ${isSubOpen ? 'open' : ''}`}
                                          size={10}
                                          strokeWidth={2.5}
                                        />
                                      </button>
                                      <button
                                        ...
                                        onContextMenu={(e) => {
                                          e.preventDefault();
                                          e.stopPropagation();
                                          setElementMenu({ x: e.clientX, y: e.clientY, element: subEra });
                                        }}
                                      >
                                        <span
                                          className={`sb-sub-era-name${selectedId === subEra.id ? ' is-selected' : ''}`}
                                          style={
                                            selectedId === subEra.id
                                              ? undefined
                          ...
                          return (
                            <>
                              <div className="sb-era-header">
                                <button
                                  className="sb-era-toggle"
                                  onClick={() => setOpenEraGroups((prev) => ({ ...prev, __other__: !isOtherOpen }))}
                                >
                                  <ChevronDown
                                    className={`sb-caret ${isOtherOpen ? 'open' : ''}`}
                                    size={11}
                ...
                <div className="sidebar-tags-dropdown">
                  ...
                  {visibleTags.map((tag) => {
                    ...
                    return (
                      ...
                      >
                        {readOnly ? (
                          <span className="sb-tag-swatch" style={{ background: tagColor || 'var(--accent-color)' }} />
                        ) : (
                          <button
                            type="button"
                          ...
                          >
                            <input
                              ref={(node) => {
                                tagColorInputRefs.current[tag] = node;
                              }}
              ...
              <div className="sidebar-groups-panel">
                <div className="sidebar-groups-list">
                  <div className="sb-group-list-header">
                    ...
                    {!readOnly && (
                      <button className="sidebar-group-add-btn" type="button" onClick={handleAddGroupAndEdit}>
                  ...
                  {visibleDisplayGroups.map((group, idx) => {
                    ...
                    const isFirstBelowLine =
                      group.belowLine && (idx === 0 || !visibleDisplayGroups[idx - 1]?.belowLine);
                    const dividerEl = (
                      <div
                      ...
                      <Fragment key={group.id}>
                        ...
                        >
                          <div
                            ...
                            onClick={() => {
                              onCenterGroup?.(group.id);
                              if (itemsInGroup.length > 0) toggleGroupContents(group.id);
                            }}
                            style={{ cursor: 'pointer' }}
                          >
                            {readOnly ? (
                              ...
                              >
                                <input
                                  ref={(node) => {
                                    groupColorInputRefs.current[group.id] = node;
                                  }}
                                  className="sidebar-group-inline-color-input"
                                  type="color"
                            ...
                            <div className="sb-group-actions" onClick={(e) => e.stopPropagation()}>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  updateGroupPatch(group.id, { visible: group.visible === false ? true : false })
                                }
                              >
                              ...
                              {!readOnly && (
                                <div
                                  className="sb-group-kebab-wrap"
                                  ref={groupMenuOpenId === group.id ? groupMenuRef : null}
                                >
                                  <button
                                    type="button"
      ...
      {!readOnly && elementMenu?.element && (
        ...
        >
          ...
          >
            <Copy size={16} />
            <span>
              Duplicate {elementMenu.element.type.charAt(0).toUpperCase() + elementMenu.element.type.slice(1)}
            </span>
          </button>
          <div className="context-menu-separator" />
          <button
