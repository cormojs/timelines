        ...
        {results.length > 0 && (
          <div className="search-overlay-results" ref={listRef}>
            {results.map((el, i) => (
              ...
              >
                <span className="search-result-type-badge" data-type={el.type}>
                  {TYPE_ICONS[el.type]}
                ...
                <span className="search-result-date">{formatElementDate(el, fileSettings)}</span>
              </button>
            ))}
          </div>
        )}

        {results.length === 0 && <div className="search-overlay-empty">No results for &ldquo;{query}&rdquo;</div>}
      </div>
    </div>
  );
}
