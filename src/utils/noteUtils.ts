...
export function sanitizeNoteHtml(
  ...
  const isPathInsideBase = (candidatePath: string): boolean => {
    ...
    if (assetsBasePath) {
      ...
      if (
        normalizedAssets &&
        (normalizedCandidate === normalizedAssets || normalizedCandidate.startsWith(`${normalizedAssets}/`))
      )
        return true;
    }
    return false;
...
export function renderNoteMarkdown(
  ...
  renderer.image = (href, title, text) => {
    ...
    if (youtubeId)
      return `<iframe class="video-embed" src="https://www.youtube-nocookie.com/embed/${youtubeId}" width="560" height="315" frameborder="0" referrerpolicy="origin-when-cross-origin" allowfullscreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>`;
